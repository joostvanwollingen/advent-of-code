...
class Day3 {
    ...
    fun solvePart2() {
        ...
        val gearSearchCoordinates = gears.map { symbol -> symbol to extractSearchCoordinates(symbol) }.sortedBy { it.first.first }
        ...
        println(gearResult.toSet().groupBy { it.first }.filter { it.value.size == 2 }.values.sumOf { it[0].second * it[1].second })
    }

    private fun getSymbolLocations(grid: Map<Int, Map<Int, Pixel>>, s: String): List<Pair<Int, Int>> {
        val symbolsByLine = grid.map { it.key to it.value.filter { pixel -> pixel.value.isSymbol && pixel.value.value == s } }.filter { it.second.isNotEmpty() }
        return symbolsByLine.map { it.second.entries.map { sm -> it.first to sm.key } }.flatten()
    }

    fun Map<Int, Map<Int, Pixel>>.getPixel(coordinates: Pair<Int, Int>): Pixel = this[coordinates.first]!![coordinates.second]!!

    fun Map<Int, Map<Int, Pixel>>.print(symbols: List<Pair<Int, Int>> = emptyList(), searchCoordinates: List<Pair<Int, Int>> = emptyList()) {
        this.entries.forEach { line ->
            print("${line.key.toString().padStart(3, '0')}: ")
            for (coord in line.value) {
                val isDiscoveredSymbol = symbols.any { symbol -> symbol.first == line.key && symbol.second == coord.key }
                val isSearched = searchCoordinates.any { symbol -> symbol.first == line.key && symbol.second == coord.key }
    ...
    private fun Pixel.toColorString(isDiscoveredSymbol: Boolean, isSearched: Boolean): String {
    ...
    private fun getSymbolLocations(grid: Map<Int, Map<Int, Pixel>>): List<Pair<Int, Int>> {
        val symbolsByLine = grid.map { it.key to it.value.filter { pixel -> pixel.value.isSymbol } }.filter { it.second.isNotEmpty() }
        return symbolsByLine.map { it.second.entries.map { sm -> it.first to sm.key } }.flatten()
    }

    private fun getNumberFromLine(map: Map<Int, Pixel>, low: Int, high: Int): Int {
        var result = ""
    ...
    private fun findConnected(grid: Map<Int, Map<Int, Pixel>>, start: Pair<Int, Int>, increment: Int, low: Int): Int {
        var lowest = low
    ...
    private fun extractSearchCoordinates(symbol: Pair<Int, Int>): List<Pair<Int, Int>> {
