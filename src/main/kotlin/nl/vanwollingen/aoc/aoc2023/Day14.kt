...
import nl.vanwollingen.aoc.util.grid.transposeToRows
import java.util.*
...
//    val input = nl.vanwollingen.aoc.util.AocUtil.load("day14.test.input")
    ...
    Day14().solvePart1(input) //broke something in funcion rollTheRocks
}
...
class Day14 {
    ...
    fun solvePart1(input: String) {
        ...
        val grid = readGrid(reflectorDish).groupBy { Day10.PointKey(it.l.y, it.l.x) }.mapValues { it.value.first() }.toMutableMap()
        grid.values.toList().print()
        grid.calcWeights()
    }

    private fun MutableMap<Day10.PointKey, Rock>.calcWeights(): Long {
    ...
    private fun getRockFromLine(index: Int, line: String): List<Rock> {
        ...
        return matches.asStream().map { match ->
            Rock(match.value, Point(index + 1, match.range.first + 1))
        }.toList()
    }
